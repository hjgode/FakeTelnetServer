    ...
    public class Screen : IAnsiDecoderClient, IEnumerable<Screen.Character>
    {
        ...
        public Point CursorPosition
        {
            ...
            set
            {
                ...
                {
                    if (CheckColumnRow(value.X, value.Y))
        ...
        public Character this[int _column, int _row]
        {
            get
            {
                ...
                if (CheckColumnRow(_column, _row))
        ...
        public string toText()
        {
            ...
            {
                StringBuilder sRow = new StringBuilder(new string(' ', Width));
                for(int col=0; col < Width; col++)
                {
                    Character character = this[col, row]; //width, height = col, row
                    if(character != null)
                        sRow[col] = character.Char;
                }//col
                tAll.Append(sRow + "\n");
            }//row
        ...
        public Bitmap ToBitmap ( Font _font )
        {
            ...
            //emSize in point
            //width in pixels
            fH = (int)(_font.GetHeight(72)+.5);
            Size textSize = System.Windows.Forms.TextRenderer.MeasureText("M", _font);
            fH = textSize.Height;
            fW = textSize.Width;

            //create a VGA image? 480 x 640?
            //cell width = 480 / WIDTH in chars
            //cell height = 640 / Height in chars
            int cellW = 480 / Width;
            int cellH = 640 / Height;
            //calculate maximal font for cell size
            System.Diagnostics.Debug.WriteLine("Width x Height="+Width.ToString()+" x "+Height.ToString()+ "\nceelW x cellH: " + cellW.ToString() + " x " + cellH.ToString());

            Bitmap bitmap = new Bitmap ( 480, 640);
            Graphics graphics = Graphics.FromImage ( bitmap );
            Font maxFont = GetAdjustedFont(graphics, ".", _font, cellW, 32, 6, true);
//            _font = maxFont; //size in points?!

            System.Diagnostics.Debug.WriteLine("maxFont: " + _font.SizeInPoints.ToString());
            //_font = new Font(_font.FontFamily, cellH, FontStyle.Regular, GraphicsUnit.Pixel);
            ...
            for ( int y = 0; y < Height; ++y )
            {
                for ( int x = 0; x < Width; ++x )
                {
                    ...
                    Rectangle rect = new Rectangle ( cellW * x, cellH * y, cellW, cellH);
                    graphics.FillRectangle ( new SolidBrush(character.Attributes.BackgroundColor), rect );
                    //System.Diagnostics.Debug.WriteLine("rect=" + rect.X.ToString() + "," + rect.Y.ToString() + "; " + rect.Width.ToString() + " x " + rect.Height.ToString());
                    Font font = _font;
                    ...
                    if ( character.Attributes.Bold )
                    {
                        ...
                        {
                            font = new Font(_font.FontFamily, _font.Size, FontStyle.Bold | FontStyle.Italic, GraphicsUnit.Pixel);
                        }
                        else
                        {
                            font = new Font(_font.FontFamily, _font.Size, FontStyle.Bold, GraphicsUnit.Pixel);
                        }
                    }
                    else if ( character.Attributes.Italic )
                    {
                        font = new Font(_font.FontFamily, _font.Size, FontStyle.Italic, GraphicsUnit.Pixel);
                    }
        ...
        Font GetAdjustedFont(Graphics graphicRef, string graphicString, Font originalFont, int containerWidth, int maxFontSize, int minFontSize, bool smallestOnFail)
        {
            Font testFont = null;
            // We utilize MeasureString which we get via a control instance           
            for (int adjustedSize = maxFontSize; adjustedSize >= minFontSize; adjustedSize--)
            {
                testFont = new Font(originalFont.Name, adjustedSize, originalFont.Style, GraphicsUnit.Pixel);
